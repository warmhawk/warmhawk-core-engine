 ...
 * (fastify, http, pg-equivalent, ioredis, etc.) — see `index.ts`, which imports this immediately
 * after `dotenv/config` and before `./app`. This file has no side effects of its own beyond
 * reading env vars until `start()` is called from `index.ts`.
 */
...
export function startOtel(): void {
  ...
  sdk = new NodeSDK({
    resource: new Resource({
      [ATTR_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || 'warmhawk-api',
    }),
 ...
 *  handler alongside `app.close()`/`prisma.$disconnect()`, so in-flight spans aren't dropped on a
 *  graceful shutdown. No-op if OTEL was never started. */
